...
+function ($) {
    ...
    Fileupload.prototype.change = function (e) {
        ...
        if (e.target.files && e.target.files[0]) {
            if ((parseInt(e.target.files[0].size) > parseInt('4219757'))) {
                $("#divErrorMsg").show();
                $("#spnErrorMsg").show();
                $("#spnErrorMsg").text("Your image is too large. Please choose an image under 4MB.");
                $(".fileinput-preview").find("img").remove().removeClass('fileinput-exists').addClass('fileinput-new');
